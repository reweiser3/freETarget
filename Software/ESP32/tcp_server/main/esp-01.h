...
bool_t       esp01_restart(void);                 // Take control and reset the device
...
bool_t       esp01_send(char* str, int channel);  // Send out a string to the channel
void         esp01_receive(void);                 // Take care of receiving characters from the IP channel
bool_t       esp01_connected(void);               // TRUE if any channel is connected.
bool_t       esp01_is_present(void);              // TRUE if an ESP-01 was found
